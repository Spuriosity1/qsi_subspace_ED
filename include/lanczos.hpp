...
concept LinearOperator = requires(const H& ham, const S* in, S* out) {
    ...
    { ham.basis.dim() } -> std::convertible_to<std::size_t>;
...
inline std::ostream& operator<<(std::ostream& os, const LanczosSettings& settings)
{
    ...
    os << "  Convergence tolerance:     " << settings.tol << "\n";
...
void tridiagonalise_lanczos(const H& ham, 
        std::vector<_S>& v, 
        ...
        )
{
    ...
    std::vector<_S> u(ham.basis.dim());
    
    assert( abs(norm(v) - 1) < settings.tol );
    ...
    for (size_t j = 0; j < settings.krylov_dim; j++) {
...
template<typename H, RealOrCplx _S>
requires LinearOperator<H, _S>
void eigval0_lanczos(const H& ham, double& eigval, 
        std::vector<_S>& v, 
        ...
        )
{
    ...
    tridiagonalise_lanczos(ham, v, alphas, betas, settings);

    std::vector<_S> ritz;
    tridiagonalise(alphas, betas, eigval, ritz);

    if(!settings.calc_eigenvector) return;

                                      
    // ----------------------------
    // Second pass: reconstruct eigenvector
    // ----------------------------
    std::vector<_S> evector(ham.basis.dim());
