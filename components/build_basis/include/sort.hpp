...
inline size_t read_batch(std::ifstream& in,
...
inline void external_mergesort_to_file(const std::vector<std::string>& shard_files,
...
inline void sort_file_in_memory(const std::string& filename, size_t num_elements) {
...
inline void sort_file_external(const std::string& filename,
                        size_t num_elements,
...
inline void sort_shard_file(const std::string& filename,
...
size_t merge_to_hdf5(const std::vector<std::string>& shard_files,
    ...
    for (auto& f : streams) {
        ...
        total_records += f.tellg() / sizeof(Uint128);
        f.seekg(0, std::ios::beg);  // Reset to beginning
    }
   ...
   // Measures and displays an execution time of a function call.
    const auto og_start{std::chrono::steady_clock::now()};
    auto start = og_start;
    ...
    while (!heap.empty()) {
        ...
        if (out_buffer.size() >= batch_size || heap.empty()) {
           ...
           const std::chrono::duration<double> total_elapsed_seconds{finish - og_start};
           start=finish;
           double completion_pct = total_merged *100.0 / total_records;
           std::cout<<"[" << std::fixed << std::setprecision(1) <<std::setw(3) << completion_pct << "%]";
           std::cout<<"  Merged "<<total_merged / batch_size <<" batches, "<< batch_size *0.001 / elapsed_seconds.count() << " krecords/s  (avg " << total_merged *0.001 / total_elapsed_seconds.count() <<")" <<std::endl;
        }
    }
