...
# XDIAG TOML EXPORT
#
# xdiag reads symmetries from a TOML file as a `Symmetries` array of
# 0-indexed permutations, optionally accompanied by irreducible
# representations stored as tables holding a `characters` list (one
# [real, imag] pair per symmetry).  See
# https://awietek.github.io/xdiag/user_guide/11-symmetries/
#
# The natural symmetry group that is always present for one of these
# supercells is the group of lattice translations, indexed by the
# integer shift (n1, n2, n3) in units of the primitive lattice vectors.
# Its irreps are the momentum (Bloch) characters.

def get_translation_group(lat: Lattice):
    """
    Enumerate the full group of primitive-lattice translations of the
    supercell as xdiag-style permutations.

    Returns (perms, shifts):
        perms[g]  -- list mapping every atom index to the atom it lands
                     on under translation shifts[g], or None if that
                     translation is not a symmetry (e.g. because some
                     sites were deleted and the shift maps an atom onto
                     a hole).
        shifts[g] -- the integer translation (n1, n2, n3), in units of
                     the primitive lattice vectors.

    The elements are ordered as itertools.product over the periodicity,
    with the identity (0, 0, 0) first.
    """
    a = lat.primitive.lattice_vectors
    perms = []
    shifts = []
    n_atoms = lat.num_atoms
    for n1, n2, n3 in lat.enumerate_primitives():
        delta = a @ Matrix([n1, n2, n3])
        perm = []
        for atom in lat.atoms:
            j = lat.as_linear_idx(atom.xyz + delta)
            if j is None:
                perm = None
                break
            perm.append(j)
        # a genuine symmetry must be a bijection of the site set
        if perm is not None and sorted(perm) != list(range(n_atoms)):
            perm = None
        perms.append(perm)
        shifts.append((n1, n2, n3))
    return perms, shifts


def _format_complex(z, tol=1e-12):
    re = 0.0 if abs(z.real) < tol else z.real
    im = 0.0 if abs(z.imag) < tol else z.imag
    return f"[{re:.15g}, {im:.15g}]"


def to_xdiag_toml(lat: Lattice, include_irreps: bool = True) -> str:
    """
    Serialise the supercell's translation symmetry group (and, by
    default, its momentum irreps) as an xdiag-compatible TOML string.

    The permutation group is written under the top-level `Symmetries`
    key.  Each momentum irrep is written as a table `[k."k1_k2_k3"]`
    holding the Bloch characters chi_k(n) = exp(2*pi*i * k.n / N) in the
    same order as `Symmetries`; load one in xdiag with the name
    e.g. `k.0_0_0`.

    Irreps are only emitted when the full translation group survives as a
    symmetry (i.e. no translations were broken by site deletions); if any
    are broken only the valid closed subgroup of translations is written,
    with a warning, and irreps are skipped.
    """
    perms, shifts = get_translation_group(lat)
    N = lat.periodicity

    full = all(p is not None for p in perms)

    if full:
        sel_perms = perms
        sel_shifts = shifts
    else:
        # keep the valid translations and verify they form a closed group
        valid = {s for s, p in zip(shifts, perms) if p is not None}
        closed = all(
            ((s1[0] + s2[0]) % N[0],
             (s1[1] + s2[1]) % N[1],
             (s1[2] + s2[2]) % N[2]) in valid
            for s1 in valid for s2 in valid)
        if not closed:
            print("WARN: surviving translations do not form a group; "
                  "exporting identity only")
            valid = {(0, 0, 0)}
        n_broken = len(shifts) - len(valid)
        print(f"WARN: {n_broken} translation(s) broken by deletions; "
              "exporting reduced group without irreps")
        sel = [(p, s) for p, s in zip(perms, shifts) if s in valid]
        sel_perms = [p for p, s in sel]
        sel_shifts = [s for p, s in sel]

    lines = []
    lines.append(f"# xdiag symmetry file, generated by lattice.py")
    lines.append(f"# supercell translation group, periodicity "
                 f"{N[0]} x {N[1]} x {N[2]} ({len(sel_perms)} elements)")
    lines.append("")
    lines.append("Symmetries = [")
    for p in sel_perms:
        lines.append("  [" + ", ".join(str(j) for j in p) + "],")
    lines.append("]")

    if include_irreps and full:
        lines.append("")
        lines.append("# momentum (Bloch) irreps: chi_k(n) = exp(2 pi i k.n / N)")
        for k1, k2, k3 in itertools.product(range(N[0]),
                                            range(N[1]),
                                            range(N[2])):
            lines.append("")
            lines.append(f'[k."{k1}_{k2}_{k3}"]')
            lines.append("characters = [")
            for (n1, n2, n3) in sel_shifts:
                phase = 2 * math.pi * (k1 * n1 / N[0]
                                       + k2 * n2 / N[1]
                                       + k3 * n3 / N[2])
                lines.append("  " + _format_complex(cmath.exp(1j * phase))
                             + ",")
            lines.append("]")
...
def export_xdiag_toml(lat: Lattice, filename: str,
                      include_irreps: bool = True):
    """Write the translation-symmetry TOML for `lat` to `filename`."""
    with open(filename, 'w') as f:
        f.write(to_xdiag_toml(lat, include_irreps=include_irreps))
